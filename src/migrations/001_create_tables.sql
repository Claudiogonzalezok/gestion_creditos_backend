...
--  Versión: 1.0 — Basada en backup real — Abril 2026
-- ============================================================
...
COMMENT ON TABLE  users                 IS 'Usuarios internos del sistema: Admin, Vendedor y Cobrador';
COMMENT ON COLUMN users.dni             IS 'Identificador usado para el login en el sistema interno';
COMMENT ON COLUMN users.is_temp_password IS 'TRUE obliga a cambiar contraseña en el próximo acceso';
COMMENT ON COLUMN users.failed_attempts IS 'Se resetea a 0 tras login exitoso. Al llegar a 3 se bloquea la cuenta';
COMMENT ON COLUMN users.locked_at       IS 'NULL = cuenta activa. Timestamp = bloqueada. Solo el Admin desbloquea';
...
COMMENT ON TABLE  customers                       IS 'Clientes con créditos activos o históricos';
COMMENT ON COLUMN customers.assigned_collector_id IS 'Cobrador responsable. Define en qué planilla diaria aparece el cliente';
COMMENT ON COLUMN customers.portal_enabled        IS 'El Admin habilita explícitamente el acceso al portal público';
...
COMMENT ON TABLE products IS 'Catálogo de productos disponibles para ventas a crédito';

CREATE INDEX idx_products_status ON products(status);

-- ============================================================
--  TABLA 4: stock_movements
-- ============================================================
CREATE TABLE public.stock_movements (
    id                    UUID            PRIMARY KEY DEFAULT gen_random_uuid(),
    product_id            UUID            NOT NULL REFERENCES products(id) ON UPDATE CASCADE,
    movement              VARCHAR(10)     NOT NULL CHECK (movement IN ('IN','OUT')),
...
COMMENT ON TABLE stock_movements IS 'Historial de movimientos de stock (entradas y salidas)';

CREATE INDEX idx_stock_movements_product ON stock_movements(product_id);
CREATE INDEX idx_stock_movements_date    ON stock_movements(created_at);

-- ============================================================
--  TABLA 5: interest_rates
-- ============================================================
...
COMMENT ON TABLE  interest_rates      IS 'Matriz de tasas configurables por el Admin. Una tasa por combinación tipo+cuotas+frecuencia';
COMMENT ON COLUMN interest_rates.rate IS 'Coeficiente multiplicador. Ej: 1.32 significa que $100.000 devuelve $132.000';
...
COMMENT ON TABLE  credits              IS 'Operación crediticia: SALE = venta de producto, LOAN = préstamo en efectivo';
COMMENT ON COLUMN credits.created_by   IS 'Vendedor o Admin que generó la operación';
COMMENT ON COLUMN credits.approved_by  IS 'Admin que aprobó o rechazó la operación';
COMMENT ON COLUMN credits.interest_rate IS 'Snapshot del coeficiente al momento de aprobación. NULL hasta ser aprobado';
...
COMMENT ON TABLE  credit_products                  IS 'Ítems de un crédito SALE con precio congelado al momento de la venta';
COMMENT ON COLUMN credit_products.historical_price IS 'Precio del producto al momento de la venta. Inmutable aunque cambie current_price';
...
COMMENT ON TABLE  installments                IS 'Cuotas del cronograma generadas al aprobar el crédito';
COMMENT ON COLUMN installments.original_amount IS 'Monto de la cuota sin mora';
COMMENT ON COLUMN installments.penalty_amount  IS 'Mora acumulada aplicada sobre esta cuota';
COMMENT ON COLUMN installments.amount_due      IS 'original_amount + penalty_amount = lo que cobra el Cobrador';
COMMENT ON COLUMN installments.amount_paid     IS 'Lo ya pagado. Se acumula con cobros parciales';
COMMENT ON COLUMN installments.payment_frequency IS 'Heredado del crédito. Define el intervalo entre vencimientos';
...
COMMENT ON TABLE  payments                    IS 'Pre-cargas de cobro: doble control Cobrador-Admin';
COMMENT ON COLUMN payments.transfer_reference IS 'Referencia bancaria para cruzar con extracto (solo en TRANSFER)';
...
COMMENT ON TABLE  cash_registers          IS 'Cierre de caja diario. Un registro por jornada. Inmutable una vez registrado';
COMMENT ON COLUMN cash_registers.difference IS 'declared_cash - cash_amount. Positivo = sobrante, negativo = faltante';
COMMENT ON COLUMN cash_registers.total_egreses IS 'Total de egresos del día (comisiones, sueldos, etc.)';

CREATE INDEX idx_cash_registers_date ON cash_registers(register_date);

-- ============================================================
--  TABLA 11: collection_sheets
-- ============================================================
...
COMMENT ON TABLE  collection_sheets            IS 'Planilla de cobro diaria por cobrador. Una planilla activa por cobrador por día';
COMMENT ON COLUMN collection_sheets.filter_used IS 'Filtro usado al generar: ALL_PENDING, OVERDUE_ONLY, etc.';
...
-- ============================================================
--  TABLA 12: collection_sheet_details
-- ============================================================
...
COMMENT ON TABLE  collection_sheet_details              IS 'Cuotas incluidas en una planilla con su orden de recorrido';
COMMENT ON COLUMN collection_sheet_details.planned_amount IS 'Snapshot del monto al momento de emitir la planilla';
...
COMMENT ON TABLE  token_blacklist         IS 'Tokens JWT invalidados. user_id para sistema interno, customer_id para portal público';
COMMENT ON COLUMN token_blacklist.token_jti IS 'Claim jti del JWT. Limpiar registros vencidos periódicamente';
...
COMMENT ON TABLE  salaries               IS 'Sueldo fijo semanal de Cobradores. Todos estandarizados a frecuencia semanal';
COMMENT ON COLUMN salaries.weekly_amount  IS 'Para cobradores que antes cobraban mensual: mensual / 4';
COMMENT ON COLUMN salaries.active         IS 'FALSE desactiva el sueldo sin eliminar el historial';

CREATE INDEX idx_salaries_user   ON salaries(user_id);
CREATE INDEX idx_salaries_active ON salaries(active);

-- ============================================================
--  TABLA 15: commissions
-- ============================================================
...
COMMENT ON TABLE  commissions        IS 'Comisiones por venta de productos. Solo créditos SALE. 8% sobre total_amount';
COMMENT ON COLUMN commissions.amount IS 'Positivo = comisión ganada. Negativo = comisión revertida por mora (REVERSED)';
...
-- ============================================================
--  TABLA 16: commission_liquidations
-- ============================================================
...
COMMENT ON TABLE  commission_liquidations                  IS 'Pago semanal del lunes: comisiones + sueldo fijo por empleado';
COMMENT ON COLUMN commission_liquidations.commissions_total IS 'Suma neta de comisiones del ciclo (puede incluir negativos por mora)';
COMMENT ON COLUMN commission_liquidations.salary_amount     IS '0 para Vendedores puros. Monto del sueldo fijo para Cobradores';
COMMENT ON COLUMN commission_liquidations.cash_register_id  IS 'Cierre de caja del lunes donde se registra este egreso';
...
COMMENT ON TABLE system_config IS 'Parámetros configurables del sistema. Solo el Admin puede modificarlos';

-- ============================================================
--  FIN DE LA MIGRACIÓN
--  Ejecutar semillas después:  npm run seed
-- ============================================================
