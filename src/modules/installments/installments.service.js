...
const applyPenalty = async (id, penaltyAmount) => {
  ...
  if (!["OVERDUE"].includes(inst.status))
    throw {
      status: 409,
      message: `No se puede aplicar mora a una cuota en estado ${inst.status}. Solo se aplica mora a cuotas OVERDUE.`,
    };

  const maxRate = parseFloat((await getValue("penalty_max_rate")) || "0.50");
  const original =
    parseFloat(inst.amount_due) - parseFloat(inst.penalty_amount);
  ...
  if (currentPenalty >= maxPenalty)
    throw {
      status: 409,
      message: `La mora ya alcanzó el tope máximo (${maxRate * 100}% del monto original).`,
    };
...
const waivePenalty = async (id) => {
  ...
  if (inst.status === "PAID")
    throw {
      status: 409,
      message:
        "La cuota ya fue cancelada. La condonación retroactiva generaría saldo a favor no soportado por el sistema.",
    };
...
const earlyPay = async (id, paymentData, adminId) => {
  const inst = await queries.findById(id);
  if (!inst) throw { status: 404, message: "Cuota no encontrada." };
  if (inst.status === "PAID")
  ...
  if (pendingAmount > 0)
    throw {
      status: 409,
      message:
        "Hay pre-cargas pendientes de aprobación para esta cuota. Aprobá o rechazalas antes de aplicar un pago anticipado.",
    };

  // Saldo real pendiente de la cuota.
  const amountReceived =
    Math.round(
      (parseFloat(inst.amount_due) - parseFloat(inst.amount_paid)) * 100,
    ) / 100;
  ...
  if (normalizedPayment.amountReceived !== amountReceived)
    throw {
      status: 409,
      message:
        "El desglose de pago no coincide con el saldo pendiente de la cuota.",
    };
