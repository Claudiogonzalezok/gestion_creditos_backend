 ...
 */
const getAccountSummary = async (customerId, graceDays = 0) => {
  const [totals, upcoming, creditsSummary] = await Promise.all([
    pool.query(
      `SELECT
         ...
         COUNT(*) FILTER (WHERE ${IS_OVERDUE_DERIVED('i', '$2')})         AS overdue_count,
         COALESCE(SUM(i.amount_paid) FILTER (WHERE i.status = 'PAID'), 0) AS total_paid_amount,
         COALESCE(SUM(i.penalty_amount) FILTER (WHERE ${IS_OVERDUE_DERIVED('i', '$2')}), 0) AS pending_penalty_amount
       FROM installments i
       JOIN credits c ON c.id = i.credit_id
       WHERE c.customer_id = $1
         AND c.status IN ('ACTIVE','SETTLED')`,
      [customerId, graceDays],
    ),
    pool.query(
      `SELECT
 ...
 */
const findCredits = async (customerId, graceDays = 0) => {
  const r = await pool.query(
    `SELECT
       ...
       COUNT(i.id) FILTER (WHERE ${IS_OVERDUE_DERIVED('i', '$2')})::int AS overdue_installments,
       ...
       COALESCE(SUM(i.penalty_amount) FILTER (WHERE ${IS_OVERDUE_DERIVED('i', '$2')}), 0)::float8
                                                            AS pending_penalty,
       BOOL_OR(${IS_OVERDUE_DERIVED('i', '$2')})            AS has_overdue,
