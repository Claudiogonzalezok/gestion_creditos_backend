...
const open = async (req, res) => {
  try {
    ...
    return res.status(201).json({ ok: true, message: 'Caja abierta correctamente.', data: session });
  } catch (err) { return handleError(res, err); }
};

const getActive = async (req, res) => {
  try {
    const ownerUserId = req.query.owner === 'me' || !req.query.owner ? req.user.id : req.query.owner;
...
const snapshot = async (req, res) => {
  try { return response.success(res, await service.snapshot(req.params.id)); }
  catch (err) { return handleError(res, err); }
};
...
const markPending = async (req, res) => {
  try {
    ...
    return res.status(200).json({ ok: true, message: 'Caja marcada como pendiente de conciliación.', data });
  } catch (err) { return handleError(res, err); }
};
...
const reconcile = async (req, res) => {
  try {
    ...
    return res.status(200).json({ ok: true, message: 'Caja conciliada.', data });
  } catch (err) { return handleError(res, err); }
};
...
const addDrop = async (req, res) => {
  try {
    ...
    return res.status(201).json({ ok: true, message: 'Drop registrado.', data });
  } catch (err) { return handleError(res, err); }
};

const reverseDrop = async (req, res) => {
  try {
    const data = await service.reverseDrop(req.params.id, req.params.dropId, req.body, req.user);
    return res.status(200).json({ ok: true, message: 'Drop revertido.', data });
  } catch (err) { return handleError(res, err); }
};

module.exports = {
  open, getActive, getById, getAll, snapshot,
  close, markPending, reconcile,
  addDrop, reverseDrop,
};
