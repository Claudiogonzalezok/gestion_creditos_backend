...
-- ── Renombrar columna a inglés (aplica a DBs existentes con cualquiera de los nombres anteriores) ───
DO $$
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE cash_registers RENAME COLUMN total_egreses TO total_outflows;
  ELSIF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'cash_registers' AND column_name = 'total_egresos'
  ) THEN
    ALTER TABLE cash_registers RENAME COLUMN total_egresos TO total_outflows;
  END IF;
END$$;
