...
describe('P — Integración Caja General (drops + commissions)', () => {
  ...
  describe('cashSessions.addDrop genera DROP_IN automático', () => {
    it('default a Caja General cuando no se pasa destination_account_id', async () => {
      ...
      const collector = await createUserFixture({ role: 'COLLECTOR', full_name: 'Juan Cobrador' });
    ...
    it('respeta destination_account_id explícito si la cuenta existe y está activa', async () => {
      ...
      const drop = await cashSessionsService.addDrop(session.id, {
        amount: 800, payment_method: 'CASH', destination_account_id: acc.id,
      }, asUser(collector));
    ...
    it('rechaza destination_account_id inexistente con 404', async () => {
      ...
      await expect(cashSessionsService.addDrop(session.id, {
        amount: 100, payment_method: 'CASH',
        destination_account_id: '00000000-0000-0000-0000-000000000000',
      }, asUser(collector))).rejects.toMatchObject({ status: 404 });
    });
    ...
    it('rechaza destination_account_id de cuenta inactiva con 404', async () => {
      ...
      try {
        await expect(cashSessionsService.addDrop(session.id, {
          amount: 100, payment_method: 'CASH', destination_account_id: acc.id,
        }, asUser(collector))).rejects.toMatchObject({ status: 404 });
      } finally {
    ...
    it('drop TRANSFER también genera DROP_IN (independiente del payment_method)', async () => {
      ...
      await cashSessionsService.addDrop(session.id, {
        amount: 700, payment_method: 'TRANSFER', receipt_reference: 'TX-001',
      }, asUser(collector));

      const movs = await cashAccountsService.listMovements(acc.id, { movementType: 'DROP_IN' });
      expect(movs.pagination.total).toBe(1);
  ...
  describe('commissions.liquidate imputa SALARY_PAYMENT a Caja General', () => {
    it('happy path: liquidación crea SALARY_PAYMENT con beneficiary y reference correctos', async () => {
      ...
      const collector = await createUserFixture({ role: 'COLLECTOR', full_name: 'Maria Cobradora' });
      ...
      const movs = await cashAccountsService.listMovements(acc.id, { movementType: 'SALARY_PAYMENT' });
      expect(movs.pagination.total).toBe(1);
    ...
    it('sin fondos en Caja General → 409 + rollback (comisión vuelve a PENDING)', async () => {
      ...
      await expect(commissionsService.liquidate({
        user_id: collector.id, payment_method: 'CASH',
      }, admin.id)).rejects.toMatchObject({
        status: 409, code: 'INSUFFICIENT_BALANCE',
      });

      // Comisión sigue PENDING; no se creó liquidation ni movement.
      const c = await pool.query(`SELECT status FROM commissions WHERE id=$1`, [commId]);
      expect(c.rows[0].status).toBe('PENDING');

      const liqs = await pool.query(`SELECT count(*)::int AS n FROM commission_liquidations`);
    ...
    it('la caja del admin no se ve afectada por liquidaciones (outflows_commissions deprecado)', async () => {
      ...
      const adminSession = await cashSessionsService.open({ opening_amount: 0 }, asUser(admin));
